...
export default function ReportsScreen() {
  const { bookings, isDark, isAr, t } = useAppContext();

  const totalWeight = bookings.reduce((sum, item) => sum + parseInt(item.unit) || 0, 0);

  return (
    <ScrollView style={[styles.container, { backgroundColor: isDark ? '#121212' : '#FFF' }]}>
      <Text style={[styles.title, { color: isDark ? '#FFF' : '#333' }]}>{isAr ? 'إحصائياتك' : t('reports')}</Text>
      ...
      <View style={styles.ecoCard}>
        ...
        <Text style={styles.ecoText}>{isAr ? 'لقد ساهمت في تقليل انبعاثات الكربون من خلال إعادة التدوير!' : 'You helped reduce carbon emissions by recycling!'}</Text>
      </View>
    </ScrollView>
  );
}
